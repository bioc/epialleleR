...
single-end alignments and has all the necessary tags (XM/XG) available. It is
...
set explicitly, exception (or warning if *`override.check=TRUE`*) is thrown
...
*`epialleleR`* provides methods *`extractPatterns`* and *`plotPatterns`* which
...
DNA.[^8] The *`generateVcfReport`* function calculates frequencies of single
